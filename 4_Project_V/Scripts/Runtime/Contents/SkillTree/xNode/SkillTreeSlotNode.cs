...
public class SkillTreeSlotNode : Node
{
    ...
    // 다른 Node의 precedingLevels 변수에 연결할 현재 Node(this)
    ...
    public bool IsSkillAcquirable(Creature creature)
    {
        // Skill 자체가 가진 습득 조건을 충족했는지 확인
        if (!skill.IsAcquirable(creature))
            return false;

        // Creature가 선행 SKill들을 가지고 있고, 선행 Skill들이 Level을 충족했는지 확인
        for (int i = 0; i < precedingLevels.Length; i++)
        {
            var inputNode = GetPrecedingSlotNode(i);
            var entitySkill = creature.SkillSystem.Find(inputNode.Skill);
    ...
    public Skill AcquireSkill(Creature creature)
    {
        Debug.Assert(IsSkillAcquirable(creature), "SkillTreeNode::AcquireSkill - Skill 습득 조건을 충족하지 못했습니다.");
        return creature.SkillSystem.Register(skill);
    }
}