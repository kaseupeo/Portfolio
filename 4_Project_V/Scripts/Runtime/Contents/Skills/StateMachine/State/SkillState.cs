...
public class SkillState : State<Skill>
{
    ...
    protected void TrySendCommandToOwner(Skill skill, CreatureStateCommand command, AnimatorParameter animatorParameter)
    {
        var ownerStateMachine = Creature.Owner.StateMachine;
        if (ownerStateMachine != null && animatorParameter.IsValid)
        {
            ...
            if (animatorParameter.type == AnimatorParameterType.Bool && ownerStateMachine.ExecuteCommand(command))
                ownerStateMachine.SendMessage(CreatureStateMessage.UsingSkill, (skill, animatorParameter));
            ...
            {
                ownerStateMachine.ExecuteCommand(CreatureStateCommand.ToDefaultState);
                ownerStateMachine.SendMessage(CreatureStateMessage.UsingSkill, (skill, animatorParameter));
            }
        }
    }
