...
namespace MyCustomEditor
{
    ...
    public class SkillTreeGraphEditor : NodeGraphEditor
    {
        ...
        private readonly int _spacingBetweenTier = 320;

        // Graph Editor를 열 때 실행되는 함수
        public override void OnOpen()
        {
            target.nodes.Remove(null);
            _nodePositions = target.nodes.Select(x => x.position).ToArray();
        }
        ...
        public override void RemoveNode(Node node)
        {
            ...
            if (target.nodes.Count == 0)
                _nodePositions = Array.Empty<Vector2>();
            else
                UpdateNodePositionsAndTiers();
        }
        ...
        private bool CheckNodePositionUpdate()
        {
            ...
            {
                if (_nodePositions[i] != target.nodes[i].position)
                    return true;
            }
        ...
        private void UpdateNodePositionsAndTiers()
        {
            ...
            for (int i = 1; i < nodes.Count; i++)
            {
                ...
                tier = (int)(Mathf.Abs(nodes[i].position.x - firstNodePosition.x) / _spacingBetweenTier);
            ...
            var indexField =
                typeof(SkillTreeSlotNode).GetField("index", BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            for (int i = 1; i < nodes.Count; i++)
            {
                ...
                if (nodesByY[i - 1].position.y - nodesByY[i].position.y >= _spacingBetweenTier)
                    index++;
